...
```{r}
# load data
X <- as.matrix(read.csv('../../../../data/simulation/setting_1/dat_X.csv'))
Y <- as.matrix(read.csv('../../../../data/simulation/setting_1/dat_Y.csv'))
loc_CV <- unlist(read.csv('../../../../data/simulation/setting_1/dat_CV.csv'))
loc_CV <- loc_CV + 1
```
...
norder <- 4 # 2, 3, 4        # order of splines (degree + 1)
n_knot <- 12 # 5, 10, 15
nbasis <- n_knot + norder - 2 
knots <- seq(y_start, y_end, length=n_knot) # knots 
Y_coef <- apply(Y_clr, 2, function(x) return(clr2coef(z, x, n_knot, y_start, y_end, d=norder-1)))
...
write.csv(quantile_val, file = '../output/qt_CLR.csv')
```
...
cdf_array <- matrix(0, nrow = 3*length(q_levs), ncol = length(p_levs))
pdf_array <- matrix(0, nrow = 3*length(q_levs), ncol = length(p_levs))
cdf_array <- matrix(0, nrow = 3*length(q_levs), ncol = length(p_levs))
